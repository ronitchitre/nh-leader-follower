...
def nonlinear_to_flat(state, state_L):
    l = state[0]
    psi = state[1]
    theta_L = state_L[2]

    y_1 = l*np.cos(psi+ theta_L - np.pi)
    y_2 = l*np.sin(psi + theta_L - np.pi)
...
def find_flat_traj(des_traj, state_L, state_L_dot):
    ...
    y_d1 = l_d * np.cos(psi_d + theta_L - np.pi)
    y_d2 = l_d * np.sin(psi_d + theta_L - np.pi)

    A = np.array([[y_d1, -1*y_d2], [y_d2, y_d1]])
    b = np.array([l_d_dot/l_d, psi_dot + theta_L_dot])
